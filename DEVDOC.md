...
against a real MariaDB 11 service, on PHP **8.1 and 8.3**. 8.1 is the floor the
README promises; 8.3 is what development uses.

There is no test suite, so CI proves the things a test suite would otherwise:

| Step | What it catches |
|---|---|
| `php -l` over every tracked file | A syntax error on the older PHP |
| `console.php doctor` | A missing extension or an unwritable upload path |
| `install --seed` | A broken schema or seeder, against a real database |
| `seed --force` again | A seeder that is not repeatable |
| Boot and fetch `/login` | Anything that only fails at runtime |
| `HEAD /login` is 200, `DELETE /login` is 405 | The two router fixes, so they cannot regress |
| `X-Frame-Options` and `X-Content-Type-Options` present | The security headers, so they cannot be dropped |

The last two rows exist because those were real bugs, not hypotheticals. See
`not_for_you.md`.

## Security headers

`public/index.php` sends `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`
and `Referrer-Policy: strict-origin-when-cross-origin` on every response, before
anything writes a body.

`DENY` matters more here than on a content site: every surface is a one-click
action behind an authenticated session, which is what clickjacking exploits.
Nothing embeds FaceClone anywhere, so there is no reason to relax it. If you ever
need to, use `frame-ancestors` in a CSP rather than loosening this.

**There is no Content-Security-Policy yet.** The views use inline styles and
inline handlers in enough places that adding one would break pages rather than
protect them; it needs a pass over the templates first.
...
`README-light.md` is generated by `scripts/build-light-readme.php`. Edit
`README.md`, run it, commit both. It exits non-zero if a marker is missing, so
the light page cannot drift into pointing at the dark screenshots. PHP rather
than Node, so the repository keeps its promise of no toolchain.
...
against `install --seed`.

Capturing needs the app in a fixed-size iframe, which `X-Frame-Options: DENY`
correctly prevents. Use a throwaway local proxy that strips the header for the
capture session. **Do not remove the header to make screenshots easier.**

## Licensing

**GPL-3.0**, see `LICENSE`. This is copyleft, not permissive: anything
distributed that is built from this must also be GPL-3.0. That differs from the
MIT projects in this account, so it is worth knowing before forking.

There are no Composer or npm dependencies to be compatible with. If you add one,
check its licence is GPL-3.0 compatible first.

## Contributors

| Who | Role |
|---|---|
| [Dileep Adari](https://github.com/Dileepadari) | Author and maintainer |
